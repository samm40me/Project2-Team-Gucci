...
(async () => {
  try {
    ...
    const directorId = directors.find(
      (director) => director.name === directorToCheck
    ).id;
    ...
    movies.forEach((movie) => {
      reviewPromises.push(
        fetch(
          "https://maciejtreder.github.io/asynchronous-javascript/movies/" +
            movie.id +
            "/reviews"
        )
          .then((response) => response.json())
          .then((reviews) => {
            return { title: movie.title, reviews: reviews };
          })
      );
    });
