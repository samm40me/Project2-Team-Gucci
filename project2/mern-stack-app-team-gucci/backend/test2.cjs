...
//    await findOneListingByName(client, "Calgary YWCA");
    ...
    await readShelterTypeAndShelter(client, {
      shelter_type: "Women Emergency",
      _shelter: "Calgary YWCA",
    });
    //     console.log(result)
...
async function readShelterTypeAndShelter(
  client, { shelter_type = "",
    _shelter = ""} = {}) {
  const cursor = client
  ...
  //          console.log(`date: ${result.date}`);
          console.log(`name: ${result.name}`);
            console.log(`sheltertype: ${result.sheltertype}`);
            console.log(`shelter: ${result.shelter}`);
        });
    } else {
        console.log(
          `No listings found `
